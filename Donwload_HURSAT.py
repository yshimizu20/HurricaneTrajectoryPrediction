...
def download_hursat(years):
    ...
    for year in years:
        ...
        year_directory_file_urls = [
            year_directory_url + "/" + node.get("href")
            for node in year_directory_soup.find_all("a")
            if node.get("href").endswith("tar.gz")
        ]
        print("\n" + year + " file loaded.")
        ...
        for storm_file_url in year_directory_file_urls:
            ...
            if file_has_match_in_best_track:
                ...
                for file_name in tar.getnames():
                    ...
                    if len(str(best_track_data["month"][0])) == 1:
                        track_month = best_track_data["month"].apply(
                            lambda x: "0" + str(x)
                        )
                    else:
                    ...
                    if len(str(best_track_data["hour"][0])) == 1:
                        track_hour = best_track_data["hour"].apply(
                            lambda x: "0" + str(x) + "00"
                        )
                    else:
                        track_hour = best_track_data["hour"].apply(
            ...
            files_processed += 1
            print_progress(
                "Processing Files for " + str(year),
                files_processed,
                len(year_directory_file_urls),
            )
