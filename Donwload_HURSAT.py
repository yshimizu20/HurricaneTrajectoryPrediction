...
# Function pulled off the web at the link below and tweaked a bit.
...
def download_hursat(years):
    # Reads in the tracking dataset from HURDAT2, which contain records of the location and maximum wind speed of every recorded
    ...
    for year in years:
        ...
        for storm_file_url in year_directory_file_urls:
            ...
            hurricane_name = storm_file_url.split('_')[-2]
            year = int(storm_file_url.split('_')[3][:4])
            file_has_match_in_best_track = not best_track_data.loc[
                (best_track_data['year'] == year) & (best_track_data['hurricane_name'] == hurricane_name)
            ].empty
